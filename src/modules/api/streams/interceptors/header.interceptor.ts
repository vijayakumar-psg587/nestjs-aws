...
export class HeaderInterceptor implements NestInterceptor {
  ...
  private async convertHeaderToMap(request: fastify.FastifyRequest): Promise<Map<String, String>> {
    ...
    //   // in a few line implementation
    //   return new Promise(async (resolve, reject) => {
    //     let headerMap = new Map();
    //
    //     const pass = new PassThrough();
    //     pass.on('data', (chunk) => {
    //       console.log('coming in data event', chunk);
    //       headerMap.set(chunk[0], chunk[1]);
    //     });
    //     pass.on('error', (err) => {
    //       console.log('Error in reading the request headers. Please restart the instance/flow');
    //       reject(AppConfigService.getCustomError('FID-HEADER', 'Invalid request/Request cannot be read - '+err['message']));
    //
    //     });
    //     pass.on('end', () => {
    //       console.log('final headerMap:', headerMap);
    //       resolve(headerMap);
    //     });
    //     const headerMapStream = await Readable.from(HeaderInterceptor.headerGenerator(request));
    //     pump(headerMapStream, pass);
    // });
  }
